...
#total zoop for ESA
total_summary <- data %>% filter(community=="DCM"|community=="DM") %>%
  group_by(temp,sample_date) %>% summarize(total = mean(total,na.rm=TRUE),
                                           se = sd(total)/sqrt(n()))

ggplot(data=filter(data,community=="DCM"|community=="DM"),aes(x=sample_date,y=total))+ 
  geom_point(color="lightgrey") +
  geom_line(aes(group = bucket),color="lightgrey") + 
  geom_point(data=total_summary,color="black") + 
  geom_line(data=total_summary,color="black")+
