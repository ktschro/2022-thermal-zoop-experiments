...
library(magrittr)
library(lubridate)
life <- read.csv("processed-data/lifetable-expt/lifetable_edit_2022.csv")
...
life %<>% mutate(inf_status = replace_na(inf, 0), dead = ifelse(is.na(REMOVED) & is.na(KBP), 1, 0)) %>% mutate(ID = paste(species, temp_id, resource, inf_status, sep = "_"))
