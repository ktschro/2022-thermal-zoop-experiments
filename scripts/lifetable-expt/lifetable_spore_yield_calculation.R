...
spore<-read.csv("Library/CloudStorage/OneDrive-SharedLibraries-UniversityofGeorgia/Strauss Lab OSE - Shared Files/Strauss Lab Sharing/Data/Lab Experiments/Life Table Spring 2022 (Suh and Schroeder)/main_mort_edit.csv")

#remove any males, any individuals that were killed by pipette, any missing individuals
spore %<>% filter(!male %in% c(1) & !missing %in% c(1) & !KBP %in% c(1) & !removed %in% c(1))
...
spore %<>% mutate(spore_yield = case_when(
  is.na(spore_RAW) ~ NA,
...
spore2<-spore %>% filter(spore_RAW>10) %>% 
...
saveRDS(spore2,"Documents/GitHub/2022-thermal-zoop-experiments/processed-data/lifetable-expt/spore_yield.rds")
